...
const [mode, setMode] = useState(“voice”); // “voice” | “text” | “image”
...
sleep: `Extract Oura sleep/recovery data from this text and return ONLY valid JSON with these optional fields: {"sleepScore": number, "readiness": number, "hoursSlept": string, "rem": string, "heartRate": number, "hrv": number, "respiratoryRate": number} hoursSlept and rem should be in H:MM format (e.g. "6:12", "1:24") if time format given, otherwise decimal. Accept any natural format like "sleep 74 readiness 68 HRV 46 HR 52 6:12 sleep 1:24 REM resp 15.2". Text: "${text}" Return only JSON, no explanation.`,
