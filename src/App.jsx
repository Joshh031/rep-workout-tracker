...
function HistoryTab({ history, setHistory, dailyLog, setDailyLog, sleepLog, setSleepLog, persist }) {
  ...
  // Merge all entries with type tag, sort newest first
  const all = [
  ...
  return (
    <div style={g.page}>
      ...
      <div style={{ marginBottom: 16 }}>
        <input
          style={{ ...g.input, fontSize: 13 }}
          placeholder="Search by date, type, score…"
          value={search}
            ...
            <div key={e.id} style={{ ...g.card, padding: "12px 14px" }}>
              ...
              {e.exercises && (
                <div style={{ fontSize: 10, color: "#666" }}>
                  {e.exercises.map(ex => ex.name).join(" · ")}
                  <span style={{ color: "#444", marginLeft: 8 }}>{e.exercises.reduce((a, ex) => a + ex.sets.length, 0)} sets</span>
                </div>
              )}
              {e.type === "run" && e.runData && (
                <div style={{ fontSize: 10, color: "#666" }}>
                  {e.runData.distance && `${e.runData.distance} mi`}{e.runData.pace && ` · ${e.runData.pace} /mi`}{e.runData.heartRate && ` · ${e.runData.heartRate} bpm`}
                </div>
              )}
            </div>
          );
