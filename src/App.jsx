...
setStatus(“parsing”);
const prompts = {
daily: `Extract daily fitness data from this voice note and return ONLY valid JSON with these optional fields: {"steps": number, "crunches": number, "planks": number, "pushups": number} Voice note: "${text}" Return only JSON, no explanation.`,
sleep: `Extract sleep/recovery data from this voice note and return ONLY valid JSON with these optional fields: {"sleepScore": number, "readiness": number, "hoursSlept": number, "rem": number, "heartRate": number, "hrv": number, "respiratoryRate": number} Voice note: "${text}" Return only JSON, no explanation.`,
workout: `Extract workout data from this voice note and return ONLY valid JSON: {"type": "chest|back|legs|shoulders|biceps|triceps|run", "exercises": [{"name": string, "sets": [{"reps": number, "weight": number}]}], "runData": {"distance": number, "duration": string, "pace": string, "heartRate": number}} For exercises, if uniform sets are mentioned (e.g. "4 sets 8 reps 225"), create that many identical set objects. Voice note: "${text}" Return only JSON, no explanation.`
};
try {
...
{[“voice”, “text”].map(m => (
<button key={m} onClick={() => { setMode(m); setStatus(””); setTranscript(””); setTextInput(””); }}
